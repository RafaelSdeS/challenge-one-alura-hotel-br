...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				calculateValue(txtDataE, txtDataS);
	...
	private void calculateValue(JDateChooser dataE, JDateChooser dataS) {
	...
	private void insertReservation() {
		if (txtDataE.getDate() != null && txtDataS.getDate() != null) {
			try {
				...
				java.sql.Date checkInDate = new java.sql.Date(txtDataE.getDate().getTime());
				java.sql.Date checkOutDate = new java.sql.Date(txtDataS.getDate().getTime());

				// Calculate the number of days of the reservation
				long diffInMillies = Math.abs(checkOutDate.getTime() - checkInDate.getTime());
				long days = (diffInMillies / (1000 * 60 * 60 * 24));

				// Calculate the reservation value (R$200 per day)
				BigDecimal ratePerDay = new BigDecimal(200);
				BigDecimal value = ratePerDay.multiply(BigDecimal.valueOf(days));

				// Create a Reservation object
				String paymentMethod = txtFormaPagamento.getSelectedItem().toString();
				Reservation reservation = new Reservation(checkInDate, checkOutDate, value, paymentMethod);

				reservationController.insertReservation(reservation);

				txtValor.setText("R$ " + value.toString());
				...
				ex.printStackTrace();
				JOptionPane.showMessageDialog(null, "Error inserting reservation: " + ex.getMessage());
			}
		} else {
